...
帳號格式（銀行端規格，不符會無法轉出檔案）：
  - 兩頁的帳號都去掉 "-" 等非數字，並在開頭補 0 補滿 14 碼。
  - 富邦（工作頁1）：14 位數字為必要條件。
  - 非富邦（工作頁2）：規格只要求不可有 "-"，補 0 到 14 碼亦可接受，故一併比照。
...
from config import HEALTH_MGMT_FEE, CROSS_BANK_FEE
...
ACCOUNT_LEN = 14  # 富邦頁帳號固定長度（不足前面補 0）；整批匯款頁不補 0
...
def _norm_account(s) -> str:
    """整批匯款（非富邦）帳號：只去掉 "-" 等非數字，**不補 0**。

    非富邦銀行的帳號長度各行不同，一律照個資檔原樣填寫即可。
    """
    return _digits(s)


def _norm_account_fubon(s) -> str:
    """富邦頁帳號：去掉 "-" 等非數字後，前面補 0 補滿 14 碼。

    富邦「客戶轉帳帳號」必須是 14 位數字才能正確轉出檔案。
    超過 14 碼不截斷（原樣回傳），由呼叫端記錄警告，避免默默匯錯帳號。
    """
    d = _digits(s)
    return d.zfill(ACCOUNT_LEN) if d else ""


def _check_account_len(p: BankPayee, acct: str, log):
    if log and acct and len(acct) != ACCOUNT_LEN:
        log(f"  ※ {p.role} {p.name} 帳號 {acct} 為 {len(acct)} 碼"
            f"（超過 {ACCOUNT_LEN} 碼，未截斷），請人工確認")
...
def _fill_fubon_sheet(ws, payees: list, month: int, log=None):
    """工作頁1：富邦整批轉帳。回傳 (筆數, 總金額)。

    轉入帳號一律正規化成 14 位數字（前面補 0）。
    """
    ...
    for i, p in enumerate(payees):
        ...
        # 填空欄位
        acct = _norm_account_fubon(p.account_number)       # 補 0 至 14 碼
        _check_account_len(p, acct, log)
        ws.cell(r, 3, acct)                                # C 轉入帳號
...
def _net_after_cross_bank_fee(amount: int) -> int:
    """跨行匯款收款人實收：扣每筆 CROSS_BANK_FEE 手續費（由收款人負擔）。
    防呆：不會扣成負數。
    """
    return max(0, int(amount) - CROSS_BANK_FEE)


def _fill_other_sheet(ws, payees: list, month: int, log=None):
    """工作頁2：跨行整批匯款。回傳 (筆數, 總金額)。

    金額扣除每筆 CROSS_BANK_FEE 跨行手續費（收款人負擔）；報稅頁不受影響。
    收款人帳號只去掉 "-" 等非數字，**不補 0**（照個資檔原樣，長度依各行）。
    """
    ...
    for i, p in enumerate(payees):
        ...
        # 填空欄位
        net = _net_after_cross_bank_fee(p.amount)  # 扣 30 元跨行手續費
        acct = _norm_account(p.account_number)     # 只去 "-"，不補 0
        ws.cell(r, 1, _digits(p.bank_code))   # A 收款行代號
        ws.cell(r, 2, acct)                   # B 收款人帳號
        ws.cell(r, 3, net)                    # C 金額（已扣手續費）
    ...
    ws.cell(SUMMARY_ROW, 3, total)     # C4 總金額（已扣手續費）
    return n, total
...
def generate_bank_transfer_file(payees: list, year: int, month: int,
    ...
    # 匯款總額：富邦行內互轉免費(原額)，跨行每筆扣 CROSS_BANK_FEE(收款人負擔)
    total_amount = (sum(p.amount for p in fubon)
                    + sum(_net_after_cross_bank_fee(p.amount) for p in other))
    stats = {
        "fubon": len(fubon), "other": len(other), "skipped": len(skipped),
        "total_amount": total_amount,
        "cross_bank_fee": CROSS_BANK_FEE,
    ...
    if SHEET_FUBON in wb.sheetnames:
        _fill_fubon_sheet(wb[SHEET_FUBON], fubon, month, log=log)
    if SHEET_OTHER in wb.sheetnames:
        _fill_other_sheet(wb[SHEET_OTHER], other, month, log=log)
