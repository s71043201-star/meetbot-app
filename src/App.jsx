...
const bdg = (c,bg) => ({ fontSize:12, padding:"2px 8px", borderRadius:20, background:bg, color:c, fontWeight:600, whiteSpace:"nowrap" });
  ...
  return (
    <div style={{ display:"flex", alignItems:"center", gap:10 }}>
      <div onClick={() => onChange(Math.max(min, value-1))} style={{
        ...
        display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", fontSize:15, userSelect:"none"
      }}>−</div>
      <span style={{ fontSize:15, fontWeight:700, minWidth:32, textAlign:"center", fontFamily:"'DM Mono',monospace" }}>{value}</span>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ fontSize:15, fontWeight:700 }}>📝 工作進度備註</div>
        <div style={{
        ...
        <textarea
          ...
          style={{
            ...
            color:"var(--text)", fontSize:15, lineHeight:1.7, padding:"14px",
        ...
        <div style={{ display:"flex", gap:10 }}>
          <button onClick={onClose} style={{
            ...
            background:"var(--surf)", color:"var(--muted)", fontSize:13, fontWeight:600,
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.75)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:"20px" }}>
      <div style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:16, padding:"24px", width:"100%", maxWidth:540, display:"flex", flexDirection:"column", gap:14, maxHeight:"90vh", overflowY:"auto" }}>
        <div style={{ fontSize:15, fontWeight:700 }}>{meeting ? "✏️ 編輯會議" : "📅 新增會議"}</div>
        <div>
          <div style={{ fontSize:12, color:"var(--muted)", marginBottom:4, fontWeight:600 }}>會議名稱</div>
          <input value={form.title} onChange={e=>setForm({...form, title:e.target.value})} placeholder="例：Q2 預算審查會議"
            style={{ width:"100%", background:"var(--surf)", border:"1px solid var(--border)", borderRadius:10, color:"var(--text)", fontSize:13, padding:"12px 14px", outline:"none", fontFamily:"inherit", boxSizing:"border-box" }}/>
        </div>
        <div style={{ display:"flex", gap:10 }}>
          <div style={{ flex:1 }}>
            <div style={{ fontSize:12, color:"var(--muted)", marginBottom:4, fontWeight:600 }}>日期</div>
            <input type="date" value={form.date} onChange={e=>setForm({...form, date:e.target.value})}
              style={{ width:"100%", background:"var(--surf)", border:"1px solid var(--border)", borderRadius:10, color:"var(--text)", fontSize:13, padding:"12px 14px", outline:"none", fontFamily:"inherit", boxSizing:"border-box" }}/>
          </div>
          <div style={{ flex:1 }}>
            <div style={{ fontSize:12, color:"var(--muted)", marginBottom:4, fontWeight:600 }}>時間</div>
            <input type="time" value={form.time} onChange={e=>setForm({...form, time:e.target.value})}
              style={{ width:"100%", background:"var(--surf)", border:"1px solid var(--border)", borderRadius:10, color:"var(--text)", fontSize:13, padding:"12px 14px", outline:"none", fontFamily:"inherit", boxSizing:"border-box" }}/>
          </div>
        </div>
        <div>
          <div style={{ fontSize:12, color:"var(--muted)", marginBottom:4, fontWeight:600 }}>地點</div>
          <input value={form.location} onChange={e=>setForm({...form, location:e.target.value})} placeholder="例：3F 會議室"
            style={{ width:"100%", background:"var(--surf)", border:"1px solid var(--border)", borderRadius:10, color:"var(--text)", fontSize:13, padding:"12px 14px", outline:"none", fontFamily:"inherit", boxSizing:"border-box" }}/>
        </div>
        <div>
          <div style={{ fontSize:12, color:"var(--muted)", marginBottom:4, fontWeight:600 }}>參與人員</div>
            ...
            {TEAM.map(name => {
              ...
              return (
                <div key={name} onClick={()=>toggleParticipant(name)} style={{
                  ...
                  color: sel ? "#fff" : "var(--muted)", cursor:"pointer", fontSize:12, fontWeight: sel ? 700 : 400,
                  transition:"all 0.2s"
                }}>
        ...
        <div>
          <div style={{ fontSize:12, color:"var(--muted)", marginBottom:4, fontWeight:600 }}>說明（選填）</div>
          <textarea value={form.description} onChange={e=>setForm({...form, description:e.target.value})} placeholder="會議討論重點..."
            style={{ width:"100%", background:"var(--surf)", border:"1px solid var(--border)", borderRadius:10, color:"var(--text)", fontSize:13, lineHeight:1.7, padding:"12px 14px", resize:"vertical", minHeight:80, fontFamily:"inherit", outline:"none", boxSizing:"border-box" }}/>
        </div>
        <div style={{ display:"flex", gap:10 }}>
          <button onClick={onClose} style={{ flex:1, padding:"13px", borderRadius:10, border:"1px solid var(--border)", background:"var(--surf)", color:"var(--muted)", fontSize:13, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>取消</button>
          <button onClick={()=>{
            if (!form.title || !form.date || !form.time) return;
            onSave(form);
          }} style={{ flex:2, padding:"13px", borderRadius:10, border:"none", background:"linear-gradient(135deg,var(--accent),#00b89c)", color:"#fff", fontSize:13, fontWeight:700, cursor:"pointer", fontFamily:"inherit" }}>
            {meeting ? "儲存修改" : "建立會議"}
          </button>
        </div>
      </div>
  ...
  if (loading) return (
    <>
      ...
      <div style={{ fontFamily:"'Noto Sans TC',sans-serif", background:"#080b12", color:"#e8eaf2", minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:18 }}>
        ...
        <div style={{ fontWeight:700, fontSize:15 }}>載入共用清單中...</div>
        ...
        <div style={{ fontSize:13, color:"#5a6285" }}>所有成員共用同一份資料</div>
      </div>
    </>
  );
    ...
    <div style={{
      ...
      <div style={{ display:"flex", gap:14, alignItems:"flex-start" }}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{
            fontSize:15, fontWeight:500, lineHeight:1.5, marginBottom:8,
          ...
          <div style={{ display:"flex", gap:8, flexWrap:"wrap", alignItems:"center", marginBottom: t.progressNote ? 8 : 0 }}>
            <div style={{ display:"flex", alignItems:"center", gap:5 }}>
              ...
              <span style={{ fontSize:13, color:"var(--muted)" }}>{t.assignee}</span>
            </div>
          ...
          {t.progressNote && (
            <div style={{ background:"rgba(79,140,255,0.07)", border:"1px solid rgba(79,140,255,0.2)", borderRadius:8, padding:"8px 12px", marginTop:4 }}>
              <div style={{ fontSize:13, color:"var(--accent)", fontWeight:600, marginBottom:3 }}>📝 進度備註</div>
              <div style={{ fontSize:13, color:"var(--text)", lineHeight:1.6 }}>{t.progressNote}</div>
              {t.progressNoteTime && <div style={{ fontSize:12, color:"var(--muted)", marginTop:4 }}>{t.progressNoteTime}</div>}
            </div>
          )}
          <div style={{ fontSize:13, color:"var(--muted)", marginTop:8 }}>來自：{t.meeting}</div>
        </div>
      </div>
      ...
      <div
        ...
        style={{
          ...
          fontSize:12, fontWeight:600, color: t.progressNote ? "var(--accent)" : "var(--muted)",
          transition:"all 0.2s"
        }}
  ...
  const DashboardContent = () => (
    <div className="mb-content-pad">
      <div style={{ background:"rgba(79,140,255,0.08)", border:"1px solid rgba(79,140,255,0.25)", borderRadius:14, padding:"12px 16px", marginBottom:14, display:"flex", alignItems:"center", gap:12 }}>
        <div style={{ fontSize:20 }}>🔗</div>
        <div>
          <div style={{ fontSize:13, fontWeight:600 }}>共用清單・即時同步</div>
          <div style={{ fontSize:13, color:"var(--muted)" }}>所有成員共用同一份資料，每 15 秒自動更新</div>
        </div>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginBottom:14 }}>
        {[{num:pendingCount,label:"待完成",color:"var(--accent)"},{num:doneCount,label:"已完成",color:"var(--green)"},{num:urgentCount,label:"緊急",color:"var(--red)"}].map(s=>(
          <div key={s.label} style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:14, padding:"14px 8px", textAlign:"center" }}>
            <div style={{ fontSize:11, fontWeight:900, fontFamily:"'DM Mono',monospace", color:s.color, lineHeight:1 }}>{s.num}</div>
      ...
      {nextReminders.length>0 && (
        <div style={{ background:"rgba(255,159,67,0.06)", border:"1px solid rgba(255,159,67,0.2)", borderRadius:14, padding:"12px 16px", marginBottom:14 }}>
          <div style={{ fontSize:13, fontWeight:700, color:"var(--orange)", marginBottom:8 }}>即將觸發的提醒</div>
          {nextReminders.slice(0,3).map((r,i)=>(
            <div key={i} style={{ display:"flex", justifyContent:"space-between", fontSize:13, color:"var(--muted)", paddingBottom:i<2?6:0, borderBottom:i<Math.min(nextReminders.length,3)-1?"1px solid var(--border)":"none", marginBottom:i<2?6:0 }}>
      ...
      <div style={{ display:"flex", gap:7, marginBottom:12, overflowX:"auto", paddingBottom:4, scrollbarWidth:"none" }}>
        {[["all","全部"],["pending","待辦"],["urgent","緊急"],["done","完成"]].map(([k,l])=>(
          <div key={k} onClick={()=>setFilter(k)} style={{ padding:"7px 14px", borderRadius:20, fontSize:13, fontWeight:600, cursor:"pointer", whiteSpace:"nowrap", background:filter===k?"var(--accent)":"var(--card)", color:filter===k?"#fff":"var(--muted)", border:`1px solid ${filter===k?"var(--accent)":"var(--border)"}`, transition:"all 0.2s" }}>{l}</div>
        ))}
        <div style={{ width:1, background:"var(--border)", margin:"0 3px", flexShrink:0 }}/>
        {["all",...TEAM].map(m=>(
          <div key={m} onClick={()=>setMemberFilter(m)} style={{ padding:"7px 14px", borderRadius:20, fontSize:13, cursor:"pointer", whiteSpace:"nowrap", background:memberFilter===m?memberColor(m):"var(--card)", color:memberFilter===m?"#fff":"var(--muted)", border:`1px solid ${memberFilter===m?memberColor(m):"var(--border)"}`, transition:"all 0.2s", fontWeight:memberFilter===m?700:400 }}>{m==="all"?"全員":m}</div>
        ))}
      </div>
      ...
      <div onClick={()=>fetchAll(true)} style={{ textAlign:"center", padding:"18px 0", fontSize:13, color:"var(--muted)", cursor:"pointer" }}>↻ 手動重新整理</div>
    </div>
  );
  ...
  const UploadContent = () => (
    <div className="mb-content-pad">
      <div style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:14, padding:"16px", marginBottom:16 }}>
        <div style={{ fontWeight:700, color:"var(--text)", marginBottom:14, fontSize:15 }}>新增單筆任務</div>
        <input
          placeholder="任務名稱"
          value={manualForm.title}
          onChange={e=>setManualForm(f=>({...f,title:e.target.value}))}
          style={{ width:"100%", padding:"12px 14px", borderRadius:10, border:"1px solid var(--border)", background:"var(--bg)", color:"var(--text)", fontSize:13, fontFamily:"inherit", marginBottom:10, outline:"none" }}
        />
        <select
          value={manualForm.assignee}
          onChange={e=>setManualForm(f=>({...f,assignee:e.target.value}))}
          style={{ width:"100%", padding:"12px 14px", borderRadius:10, border:"1px solid var(--border)", background:"var(--bg)", color:"var(--text)", fontSize:13, fontFamily:"inherit", marginBottom:10 }}
        >
        ...
        <input
          ...
          style={{ width:"100%", padding:"12px 14px", borderRadius:10, border:"1px solid var(--border)", background:"var(--bg)", color:"var(--text)", fontSize:13, fontFamily:"inherit", marginBottom:14, outline:"none" }}
        />
        <button onClick={addManualTask} style={{ width:"100%", padding:"14px", borderRadius:12, border:"none", background:"linear-gradient(135deg,var(--accent),#7c5fe6)", color:"#fff", fontSize:15, fontWeight:700, cursor:"pointer", fontFamily:"inherit" }}>新增任務</button>
      </div>
      <div onClick={()=>!parsing&&fileRef.current.click()} style={{ border:`2px dashed ${parsing?"var(--accent)":"var(--border)"}`, borderRadius:16, padding:"36px 20px", textAlign:"center", cursor:"pointer", background:"var(--card)", marginBottom:16, transition:"border-color 0.2s" }}>
        ...
        {parsing ? (<>
          <div style={{ fontSize:11, marginBottom:12 }}>⚙️</div>
          <div style={{ fontWeight:700, fontSize:15, marginBottom:6 }}>AI 解析中...</div>
          <div style={{ fontSize:13, color:"var(--muted)" }}>正在從會議紀錄提取任務</div>
          ...
          <div style={{ fontWeight:700, fontSize:15, marginBottom:6 }}>{docName||"點擊上傳 .docx 會議紀錄"}</div>
          <div style={{ fontSize:13, color:"var(--muted)" }}>AI 自動解析任務、負責人、截止日期</div>
        </>)}
      </div>
      {parseResult && (
        <div style={{ animation:"fadeUp 0.4s ease" }}>
          <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:12, fontSize:13, color:"var(--green)", fontWeight:700 }}>找到 {parseResult.length} 項任務，確認後同步給全團隊</div>
          {parseResult.map((t,i)=>(
            <div key={i} style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:14, padding:"14px 16px", marginBottom:10 }}>
              <div style={{ fontSize:15, fontWeight:500, marginBottom:8 }}>{t.title}</div>
              <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                <div style={{ display:"flex", alignItems:"center", gap:5 }}><Avatar name={t.assignee} size={22}/><span style={{ fontSize:13, color:"var(--muted)" }}>{t.assignee}</span></div>
                <span style={bdg("var(--orange)","rgba(255,159,67,0.1)")}>📅 {t.deadline}</span>
              </div>
            </div>
          ))}
          <button onClick={confirmTasks} style={{ width:"100%", padding:"16px", borderRadius:12, border:"none", background:"linear-gradient(135deg,var(--green),#00b89c)", color:"#fff", fontSize:15, fontWeight:700, cursor:"pointer", fontFamily:"inherit", boxShadow:"0 4px 20px rgba(0,229,195,0.3)", marginTop:4 }}>同步給全團隊</button>
        </div>
      )}
      {!parsing&&!parseResult && (
        <div style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:14, padding:"16px", fontSize:13, color:"var(--muted)", lineHeight:2 }}>
          <div style={{ fontWeight:700, color:"var(--text)", marginBottom:8, fontSize:15 }}>使用說明</div>
  ...
  const TeamContent = () => (
    <div className="mb-content-pad">
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
        <div style={{ fontSize:13, color:"var(--muted)", fontWeight:700, letterSpacing:1.5, textTransform:"uppercase" }}>成員完成率（即時）</div>
          ...
          <div key={m.name} className="mb-member-card" style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:14, padding:"16px" }}>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:12 }}>
              <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                ...
                <div><div style={{ fontWeight:700, fontSize:15 }}>{m.name}</div><div style={{ fontSize:13, color:"var(--muted)" }}>{m.done}/{m.total} 項完成</div></div>
              </div>
            ...
            <div>
              {tasks.filter(t=>t.assignee===m.name&&!t.done).slice(0,3).map(t=>(
                <div key={t.id} style={{ fontSize:13, color:"var(--muted)", padding:"6px 0", borderTop:"1px solid var(--border)", display:"flex", justifyContent:"space-between", gap:8, alignItems:"flex-start" }}>
                  <div style={{ flex:1 }}>
                    <div style={{ overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>• {t.title}</div>
                    {t.progressNote && <div style={{ fontSize:12, color:"var(--accent)", marginTop:3 }}>📝 {t.progressNote.length>30?t.progressNote.slice(0,30)+"…":t.progressNote}</div>}
                  </div>
      ...
      <div style={{ fontSize:13, color:"var(--muted)", fontWeight:700, letterSpacing:1.5, textTransform:"uppercase", margin:"22px 0 12px" }}>固定成員</div>
        ...
        <div style={{ display:"flex", flexWrap:"wrap", gap:10 }}>
          {TEAM.map(name=>(
            <div key={name} style={{ display:"flex", alignItems:"center", gap:7, background:"var(--surf)", borderRadius:24, padding:"6px 14px 6px 7px" }}>
              <Avatar name={name} size={26}/><span style={{ fontSize:13 }}>{name}</span>
            </div>
          ))}
        </div>
      </div>
  ...
  const RemindersContent = () => (
    <div className="mb-content-pad">
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16 }}>
        <div style={{ fontSize:13, color:"var(--muted)", fontWeight:700, letterSpacing:1.5, textTransform:"uppercase" }}>提醒規則設定</div>
        <div style={{ fontSize:13, color: savedPulse?"var(--green)":"var(--muted)", fontWeight:600, transition:"color 0.3s", animation: savedPulse?"savedPop 0.4s ease":undefined }}>
      ...
      <div style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:14, padding:"18px", marginBottom:12 }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:reminders.dayBefore.on?16:0 }}>
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            <div style={{ width:40, height:40, borderRadius:10, background:"rgba(79,140,255,0.12)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:15 }}>📅</div>
            <div><div style={{ fontSize:15, fontWeight:600 }}>截止日前提醒</div><div style={{ fontSize:13, color:"var(--muted)" }}>在截止日的前幾天早上提醒</div></div>
          </div>
            ...
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:8 }}>
              ...
              <div style={{ display:"flex", gap:7, flexWrap:"wrap" }}>
                {[7,8,9,10,12,14].map(h=>(
                  <div key={h} onClick={()=>updateReminder("dayBefore",{hour:h})} style={{ padding:"5px 12px", borderRadius:8, fontSize:13, cursor:"pointer", fontFamily:"'DM Mono',monospace", background:reminders.dayBefore.hour===h?"var(--accent)":"var(--surf)", color:reminders.dayBefore.hour===h?"#fff":"var(--muted)", border:`1px solid ${reminders.dayBefore.hour===h?"var(--accent)":"var(--border)"}`, transition:"all 0.2s" }}>{pad2(h)}:00</div>
                ))}
              </div>
            </div>
            <div style={{ background:"var(--surf)", borderRadius:8, padding:"10px 14px", fontSize:13, color:"var(--muted)" }}>
      ...
      <div style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:14, padding:"18px", marginBottom:12 }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:reminders.hourBefore.on?16:0 }}>
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            <div style={{ width:40, height:40, borderRadius:10, background:"rgba(255,159,67,0.12)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:15 }}>⏰</div>
            <div><div style={{ fontSize:15, fontWeight:600 }}>截止前緊急提醒</div><div style={{ fontSize:13, color:"var(--muted)" }}>截止前幾小時發出最後警示</div></div>
          </div>
      ...
      <div style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:14, padding:"18px", marginBottom:12 }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            <div style={{ width:40, height:40, borderRadius:10, background:"rgba(255,91,121,0.12)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:15 }}>🚨</div>
            <div><div style={{ fontSize:15, fontWeight:600 }}>逾期高亮提示</div><div style={{ fontSize:13, color:"var(--muted)" }}>逾期任務在儀表板醒目標示</div></div>
          </div>
      ...
      <div style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:14, padding:"18px", marginBottom:12 }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:reminders.weeklyReport.on?16:0 }}>
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            <div style={{ width:40, height:40, borderRadius:10, background:"rgba(0,229,195,0.12)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:15 }}>📊</div>
            <div><div style={{ fontSize:15, fontWeight:600 }}>每週完成率報告</div><div style={{ fontSize:13, color:"var(--muted)" }}>固定時間推播團隊整體進度</div></div>
          </div>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:14, paddingTop:16, borderTop:"1px solid var(--border)" }}>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
              ...
              <div style={{ display:"flex", gap:6 }}>
                {[1,2,3,4,5].map(d=>(
                  <div key={d} onClick={()=>updateReminder("weeklyReport",{weekday:d})} style={{ width:38, height:38, borderRadius:8, display:"flex", alignItems:"center", justifyContent:"center", fontSize:13, cursor:"pointer", background:reminders.weeklyReport.weekday===d?"var(--accent)":"var(--surf)", color:reminders.weeklyReport.weekday===d?"#fff":"var(--muted)", border:`1px solid ${reminders.weeklyReport.weekday===d?"var(--accent)":"var(--border)"}`, transition:"all 0.2s" }}>週{WEEKDAYS[d]}</div>
                ))}
              </div>
            </div>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:8 }}>
              ...
              <div style={{ display:"flex", gap:7 }}>
                {[9,12,14,16,17,18].map(h=>(
                  <div key={h} onClick={()=>updateReminder("weeklyReport",{hour:h})} style={{ padding:"5px 12px", borderRadius:8, fontSize:13, cursor:"pointer", fontFamily:"'DM Mono',monospace", background:reminders.weeklyReport.hour===h?"var(--green)":"var(--surf)", color:reminders.weeklyReport.hour===h?"#fff":"var(--muted)", border:`1px solid ${reminders.weeklyReport.hour===h?"var(--green)":"var(--border)"}`, transition:"all 0.2s" }}>{pad2(h)}:00</div>
                ))}
              </div>
            </div>
            <div style={{ background:"var(--surf)", borderRadius:8, padding:"10px 14px", fontSize:13, color:"var(--muted)" }}>
      ...
      {nextReminders.length>0 && (
        <div style={{ marginTop:22 }}>
          <div style={{ fontSize:13, color:"var(--muted)", fontWeight:700, letterSpacing:1.5, textTransform:"uppercase", marginBottom:12 }}>依目前設定，即將提醒</div>
          {nextReminders.map((r,i)=>(
            <div key={i} style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:10, padding:"13px 16px", marginBottom:7, display:"flex", alignItems:"center", justifyContent:"space-between", gap:12 }}>
              <div style={{ flex:1, minWidth:0 }}>
                ...
                <div style={{ fontSize:13, color:"var(--muted)", marginTop:3 }}>{r.task.assignee} · {r.type}</div>
              </div>
              <div style={{ fontSize:13, color:"var(--orange)", fontWeight:700, fontFamily:"'DM Mono',monospace", whiteSpace:"nowrap" }}>
      ...
      <div style={{ background:"rgba(79,140,255,0.06)", border:"1px solid rgba(79,140,255,0.2)", borderRadius:14, padding:"16px", marginTop:18, fontSize:13, color:"var(--muted)", lineHeight:1.9 }}>
        <div style={{ fontWeight:700, color:"var(--text)", marginBottom:8, fontSize:15 }}>自動提醒說明</div>
      ...
      }} style={{ width:"100%", padding:"15px", borderRadius:12, border:"1px solid var(--border)", background:"var(--card)", color:"var(--text)", fontSize:13, fontWeight:600, cursor:"pointer", fontFamily:"inherit", marginTop:12 }}>
        立即檢查並發送 LINE 提醒
      </button>
      {lastNotify && <div style={{ textAlign:"center", fontSize:13, color:"var(--muted)", marginTop:10 }}>上次發送：{pad2(lastNotify.getHours())}:{pad2(lastNotify.getMinutes())}</div>}
    </div>
  );
    ...
    <div className="mb-content-pad">
        ...
        <div style={{ display:"flex", gap:6 }}>
          {[["month","月曆"],["timeline","時間軸"]].map(([v,l])=>(
            <button key={v} onClick={()=>setCalView(v)} style={{
              padding:"7px 18px", borderRadius:20, fontSize:12, fontWeight: calView===v ? 700 : 400,
      ...
      {calView==="month" && (<>
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:20, marginBottom:14 }}>
          <div onClick={prevMonth} style={{ width:36, height:36, borderRadius:10, background:"var(--card)", border:"1px solid var(--border)", display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", fontSize:13 }}>‹</div>
          <div style={{ fontSize:15, fontWeight:700, minWidth:160, textAlign:"center" }}>{calMonth.year} 年 {calMonth.month+1} 月</div>
          <div onClick={nextMonth} style={{ width:36, height:36, borderRadius:10, background:"var(--card)", border:"1px solid var(--border)", display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", fontSize:13 }}>›</div>
        </div>

        {/* 星期標頭 */}
        <div style={{ display:"grid", gridTemplateColumns:"repeat(7, 1fr)", gap:2, marginBottom:4 }}>
          {WEEKDAY_LABELS.map(w => (
            <div key={w} style={{ textAlign:"center", fontSize:11, color:"var(--muted)", fontWeight:700, padding:"6px 0" }}>{w}</div>
          ))}
        </div>
          ...
          <div style={{ marginTop:14, animation:"fadeUp 0.3s ease" }}>
              ...
              <div key={m.id} style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:14, padding:"14px 16px", marginBottom:8 }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                  <div>
                    ...
                    <div style={{ fontSize:12, color:"var(--muted)" }}>⏰ {m.time} &nbsp; 📍 {m.location||"未指定"}</div>
                    {m.description && <div style={{ fontSize:12, color:"var(--muted)", marginTop:6, lineHeight:1.6 }}>{m.description}</div>}
                  </div>
                  <div style={{ display:"flex", gap:6, flexShrink:0 }}>
                    <div onClick={()=>{ setEditingMeeting(m); setShowMeetingModal(true); }} style={{ padding:"4px 10px", cursor:"pointer", color:"var(--accent)", fontSize:12, fontWeight:600 }}>✎</div>
                    <div onClick={()=>removeMeeting(m.id)} style={{ padding:"4px 10px", cursor:"pointer", color:"var(--red)", fontSize:12, fontWeight:600 }}>✕</div>
                  </div>
                </div>
                {m.participants?.length > 0 && (
                  <div style={{ display:"flex", gap:6, marginTop:8, flexWrap:"wrap" }}>
                    {m.participants.map(p => (
                      <div key={p} style={{ display:"flex", alignItems:"center", gap:4, fontSize:11, color:"var(--muted)" }}>
            ...
            {tasks.filter(t => t.deadline === selectedDate && !t.done).map(t => (
              <div key={t.id} style={{ background:"var(--card)", border:"1px solid rgba(255,159,67,0.3)", borderRadius:14, padding:"12px 16px", marginBottom:8 }}>
                <div style={{ fontSize:12, color:"var(--orange)", fontWeight:600, marginBottom:2 }}>📋 任務截止</div>
                <div style={{ fontSize:13, fontWeight:500 }}>{t.title}</div>
                <div style={{ fontSize:11, color:"var(--muted)", marginTop:4 }}>{t.assignee}</div>
              </div>
            ))}
            {!(meetingsByDate[selectedDate]||[]).length && !tasks.filter(t=>t.deadline===selectedDate&&!t.done).length && (
              <div style={{ textAlign:"center", color:"var(--muted)", padding:"20px 0", fontSize:13 }}>這天沒有行程</div>
            )}
          </div>
        )}
      </>)}
      ...
      {calView==="timeline" && (
        <div>
          <div style={{ fontSize:13, color:"var(--muted)", fontWeight:700, letterSpacing:1.5, marginBottom:14 }}>近期會議</div>
          {meetings.filter(m => m.date >= today()).sort((a,b) => a.date.localeCompare(b.date) || (a.time||"").localeCompare(b.time||"")).length === 0 && (
            <div style={{ textAlign:"center", color:"var(--muted)", padding:"40px 0", fontSize:13 }}>尚無近期會議</div>
          )}
                    ...
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                      <div style={{ flex:1 }}>
                        <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:6 }}>
                          <span style={{ fontSize:11, padding:"3px 10px", borderRadius:12, background:`${countdownColor}18`, color:countdownColor, fontWeight:700 }}>{countdownText}</span>
                          <span style={{ fontSize:11, color:"var(--muted)" }}>{m.date.slice(5).replace("-","/")} {m.time}</span>
                        </div>
                        <div style={{ fontSize:15, fontWeight:600, marginBottom:4 }}>{m.title}</div>
                        <div style={{ fontSize:12, color:"var(--muted)" }}>📍 {m.location||"未指定地點"}</div>
                        {m.description && <div style={{ fontSize:11, color:"var(--muted)", marginTop:4, lineHeight:1.6, display:"-webkit-box", WebkitLineClamp:2, WebkitBoxOrient:"vertical", overflow:"hidden" }}>{m.description}</div>}
                      </div>
                      <div style={{ display:"flex", gap:6, flexShrink:0 }}>
                        <div onClick={()=>{ setEditingMeeting(m); setShowMeetingModal(true); }} style={{ padding:"6px", cursor:"pointer", color:"var(--accent)", fontSize:12 }}>✎</div>
                        <div onClick={()=>removeMeeting(m.id)} style={{ padding:"6px", cursor:"pointer", color:"var(--red)", fontSize:12 }}>✕</div>
                      </div>
                    </div>
                    {m.participants?.length > 0 && (
                      <div style={{ display:"flex", gap:6, marginTop:8, flexWrap:"wrap" }}>
                        {m.participants.map(p => <div key={p} style={{ display:"flex", alignItems:"center", gap:4, fontSize:11, color:"var(--muted)" }}><Avatar name={p} size={18}/>{p}</div>)}
                      </div>
                    )}
                    {/* Slack 提醒狀態 */}
                    <div style={{ display:"flex", gap:8, marginTop:8, flexWrap:"wrap" }}>
                      {[["day7","7天前"],["day3","3天前"],["day1","1天前"]].map(([k,l])=>(
                        <span key={k} style={{ fontSize:10, padding:"2px 8px", borderRadius:10, background: m.slackSent?.[k] ? "rgba(0,229,195,0.12)" : "rgba(107,116,148,0.08)", color: m.slackSent?.[k] ? "var(--green)" : "var(--muted)" }}>
                          {m.slackSent?.[k] ? "✓" : "○"} {l}
                        </span>
                      ))}
                    </div>
          ...
          {meetings.filter(m => m.date < today()).length > 0 && (
            <div style={{ marginTop:20 }}>
              <div style={{ fontSize:13, color:"var(--muted)", fontWeight:700, letterSpacing:1.5, marginBottom:14 }}>已過期會議</div>
                ...
                <div key={m.id} style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:14, padding:"14px 16px", marginBottom:8, opacity:0.5 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                    <div>
                      ...
                      <div style={{ fontSize:11, color:"var(--muted)" }}>{m.date} {m.time} · {m.location||""}</div>
                    </div>
                    <div onClick={()=>removeMeeting(m.id)} style={{ padding:"6px", cursor:"pointer", color:"var(--red)", fontSize:11 }}>✕</div>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div style={{ background:"var(--card)", border:"1px solid var(--border)", borderRadius:14, padding:"16px", marginTop:20 }}>
        <div style={{ fontSize:13, fontWeight:700, marginBottom:10, display:"flex", alignItems:"center", gap:8 }}>
          <span style={{ fontSize:15 }}>💬</span> Slack 會議提醒設定
        </div>
        ...
        <input
          ...
          style={{ width:"100%", background:"var(--surf)", border:"1px solid var(--border)", borderRadius:10, color:"var(--text)", fontSize:12, padding:"12px 14px", outline:"none", fontFamily:"inherit", marginBottom:10, boxSizing:"border-box" }}
        />
        <div style={{ display:"flex", gap:8 }}>
          <button onClick={testSlack} style={{
            flex:1, padding:"11px", borderRadius:10, border:"1px solid var(--border)",
            background:"var(--surf)", color:"var(--text)", fontSize:12, fontWeight:600,
            cursor:"pointer", fontFamily:"inherit"
          }}>🔔 測試發送</button>
          <button onClick={async () => {
            ...
            background:"linear-gradient(135deg,var(--accent),#00b89c)", color:"#fff", fontSize:12, fontWeight:700,
            cursor:"pointer", fontFamily:"inherit"
          }}>📨 立即檢查提醒</button>
        </div>
      </div>
    ...
    <>
      ...
      <div ref={rootRef} className={isWide ? "mb-root mb-wide" : "mb-root"} style={{ fontFamily:"'Noto Sans TC',sans-serif", background:"var(--bg)", color:"var(--text)" }}>
        ...
        <div className="mb-topbar-inner" style={{ background:"var(--surf)", borderBottom:"1px solid var(--border)", padding:"13px 16px", display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, zIndex:50 }}>
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            <div className="mb-topbar-logo" style={{ width:36, height:36, borderRadius:10, background:"linear-gradient(135deg,#4f8cff,#00e5c3)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:14 }}>📋</div>
            <div>
              <div className="mb-topbar-title" style={{ fontWeight:700, fontSize:15 }}>MeetBot</div>
              <div className="mb-topbar-sub" style={{ fontSize:12, color:"var(--muted)" }}>會議任務追蹤系統</div>
            </div>
          </div>
          <div style={{ display:"flex", alignItems:"center", gap:10, flexWrap:"wrap", justifyContent:"flex-end" }}>
            {urgentCount>0 && <div style={{ background:"rgba(255,91,121,0.15)", border:"1px solid var(--red)", color:"var(--red)", fontSize:12, fontWeight:700, padding:"3px 12px", borderRadius:20 }}>緊急 {urgentCount} 項</div>}
            <div style={{ background:"rgba(0,229,195,0.1)", border:"1px solid rgba(0,229,195,0.3)", color:"var(--green)", fontSize:12, fontWeight:700, padding:"3px 12px", borderRadius:20 }}>{pct}% 完成</div>
            <button onClick={()=>exportToWord(tasks)} style={{
            ...
            <div style={{ display:"flex", alignItems:"center", gap:5, fontSize:12, color: syncing?"var(--accent)":"var(--muted)" }}>
        ...
        <div className="mb-main">
          ...
          <div className="mb-content-area" style={{ flex:1, minWidth:0 }}>
            ...
            <div className="mb-tabs">
              {TABS.map(([id,ic,lb])=>(
                <div key={id} onClick={()=>setTab(id)} style={{
                  flex:1, minWidth:68, textAlign:"center", padding:"10px 4px 8px", fontSize:12, cursor:"pointer",
                  color: tab===id ? "var(--accent)" : "var(--muted)",
                ...
                }}><div style={{ fontSize:15, marginBottom:2 }}>{ic}</div>{lb}</div>
              ))}
            </div>
        ...
        {/* Toast */}
        {toast && (
          <div style={{ position:"fixed", bottom:28, left:"50%", transform:"translateX(-50%)", background:toast.color, color:"#fff", padding:"12px 24px", borderRadius:24, fontSize:13, fontWeight:600, zIndex:999, whiteSpace:"nowrap", boxShadow:"0 4px 20px rgba(0,0,0,0.4)", animation:"fadeUp 0.3s ease" }}>{toast.msg}</div>
        )}
      </div>
    </>
  );
